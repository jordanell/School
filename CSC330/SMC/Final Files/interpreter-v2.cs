...
public class Interpreter {
        ...
        try {
            ...
            for( ; ; ) {
                ...
                case OpCode.CmpLe:
                    ...
                    else if (val1.t == TypeCode.StringType && val2.t == TypeCode.StringType)
                        doStrOp(ins.Op, val1.strValue, val2.strValue);
                    else if (val1.t == TypeCode.StringType && val2.t == TypeCode.IntType)
                        doStrOp(ins.Op, val1.strValue, val2.intValue);
                    else if (val1.t == TypeCode.IntType && val2.t == TypeCode.StringType)
                        doStrOp(ins.Op, val2.strValue, val1.intValue);
                    else
                    {
    ...
    void doStrOp(OpCode op, string opnd1, string opnd2)
    {
        switch (op)
        {
            case OpCode.Add: push(opnd1 + opnd2); break;
            default:
                throw new RunTimeError("Invalid string operator");
        }
    }

    void doStrOp(OpCode op, string opnd1, int opnd2)
    {
        switch (op)
        {
            case OpCode.Mul:
