...
class Parser {
    ...
    public class Function
    {
        public string name;
        public int args;
        public Function(string s, int i)
        {
            name = s;
            args = i;
        }
    ...
    public void checkFunctionCalls()
    {
        ...
        foreach(Function f in calls)
        {
            ...
            {
                if (fn.name == name && fn.args == argCount)
                    valid = 1;
            }

            if (valid == 0)
                throw new ParseError("An undefined function was called: " + name);
    ...
    void matchAssignmentOrCall() {
        ...
        if (nextToken == Token.LPar) {
            ...
            if (target != "Read" && target != "Write" && target != "WriteLine")
            {
                Function fn = new Function(target, argCnt);
                calls.Add(fn);
            }
